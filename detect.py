...
def detect(save_img=False):
    ...
    if classify:
        ...
        modelc.set_state_dict(
            state_dict=paddle.load(path="weights/resnet101.pt")["model"]
        )
    ...
    for path, img, im0s, vid_cap in dataset:
        img = paddle.to_tensor(data=img).to(device)
        ...
        for i, det in enumerate(pred):
            ...
            s += "%gx%g " % img.shape[2:]
            gn = paddle.to_tensor(data=im0.shape)[[1, 0, 1, 0]]
